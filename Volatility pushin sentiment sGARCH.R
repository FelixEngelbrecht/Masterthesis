...
df = na.omit(df)
acf(df$close)
ccf(df$close, df$pos_Sent_mva)
ccf(df$close, df$neg_Sent_mva)


close = df[,4]
plot.ts(close)
close_rets = diff(log(close))
plot.ts(close_rets, main = "Daily log returns", xlab = "D
        ate", ylab = "Return in percent")

pos_Sent_mva= df[,5]
pos_Sent = df[,2]
plot.ts(pos_Sent_mva)
plot.ts(pos_Sent)
neg_Sent_mva = df[,6]
neg_Sent = df[,3]
plot.ts(neg_Sent_mva)
plot.ts(neg_Sent)

acf(close_rets)
ccf(close_rets, pos_Sent_mva)
ccf(close_rets, neg_Sent_mva)
...
adf.test(df$pos_Sent_mva)
adf.test(df$neg_Sent_mva)
